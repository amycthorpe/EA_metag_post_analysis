#!/hdd0/susbus/tools/conda_env/359b7172c00f55ffb49ceca1140d208f_/bin/python
...
output_dir = os.path.join(OUTPUT, "out")
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

all_dicts={}
result_dirs = os.walk(output_dir)
for path, dirs, files in result_dirs:
    genome_dict={}
    for file in files:
        genome = file.split("-")[0]
        prot = file.replace(".out", "").split("-")[1]
        result = output_dir+"/"+genome+"/"+file
...
out_stats = out_results +"/"+ OUTFILE
df_final.to_csv(out_stats)

if PLOTTING == 'ON':
    METADATA = args.metadata
    FIGOUT = args.heatmap
    FIGPATH = out_results+"/"+ FIGOUT
    AGG = args.aggregate
    ORDER = args.ordering
    ...
    clean[clean>1] = 1
    out_clean = out_results + "/cleaned-matrix.csv"
    ...
    if AGG == 'OFF':
        ...
        if ORDER == 'OFF':
            ...
            drop = merged.drop('genome', axis=1)
